...
class AbstractBot(ABC):
    def listen_optional_attack(self, attacker_index: int, card_list: List[Tuple[int, int]]):
        """Listen to a non-starting attack from a player."""
        pass
    def listen_first_attack(self, attacker_index: int, card_list: List[Tuple[int, int]]):
        """Listen to the starting attack from a player."""
        pass
    def listen_defence(self, defender_index: int, defending_cards: List[Tuple[int, int]], indexes: List[int]):
    ...
    def listen_winner(self, winner_index: int):
        ...
        pass
    def game_init(self, num_of_players: int, my_index: int, hand: List[Tuple[int, int]], kozar_card: Tuple[int, int]):
    ...
    def get_original_index(self, current_index: int) -> int:
        ...
        if not self.active_players:
            return -1
        if current_index < 0 or current_index >= len(self.active_players):
            return -1
        return self.active_players[current_index]
    def get_current_index(self, original_index: int) -> int:
        """Get the current index of a player based on its original index.
        If something is wrong, return -1."""
        if not self.active_players:
            return -1
        if original_index not in self.active_players:
            return -1
        return self.active_players.index(original_index)
    ...
    def get_kozar_suit(self) -> int:
        ...
        return self.kozar_card[1] if self.kozar_card else -1
    def get_kozar_card(self) -> Tuple[int, int]:
        """Get the kozar card."""
        return self.kozar_card if self.kozar_card else (-1, -1)
    def get_attacking_table_cards(self) -> List[Tuple[int, int]]:
        """Get the current attacking cards on the table."""
        return self.table_cards[0]
    def get_defending_table_cards(self) -> List[Tuple[int, int]]:
    ...
    def get_my_index(self) -> int:
        ...
        return self.my_index if hasattr(self, 'my_index') else -1

    def __call__(self, event: Tuple, hand: List, table_cards: List, state: Dict[str, Any]):
        ...
        match action:
            ...
            case Input_actions.GAME_INIT:
                self.active_players = [i for i in range(event[1])]
                self.my_index = event[2]
                self.hand = event[3]
                self.kozar_card = event[4]
                self.game_init(event[1], event[2], event[3], event[4])
            case Input_actions.WINNER_PASSIVE:
                self.active_players.pop(event[1])
